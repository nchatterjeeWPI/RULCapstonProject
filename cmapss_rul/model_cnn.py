...
from tensorflow.keras.layers import Input, Conv1D, Dense, Dropout, GlobalAveragePooling1D, MaxPooling1D, BatchNormalization, Activation
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.callbacks import EarlyStopping

def _conv_block(x, filters, kernel_size, pool=True, dropout=0.0):
...
def build_model(hp, input_shape):
    ...
    for i in range(blocks):
        x = _conv_block(x, filters, kernel_size, pool=True, dropout=dropout)
...
def tune_and_train(X_tr, y_tr, X_val, y_val, max_epochs=60, directory='tuning', project_name='cnn'):
    tuner = kt.Hyperband(
