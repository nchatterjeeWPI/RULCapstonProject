...
def build_feature_cols(df: pd.DataFrame, regime_K: int) -> List[str]:
    setting_cols = [c for c in df.columns if c.startswith(('op_setting_', 'setting_'))]
    sensor_cols  = [c for c in df.columns if c.startswith('sensor_')]
    regime_onehot = [f"regime_{r}" for r in range(regime_K)]
    return sensor_cols + setting_cols + regime_onehot
...
def build_test_sequences_per_dataset(test_data_dict, seq_len, feature_cols):
    import numpy as np
    ...
    for fd, df in test_data_dict.items():
        ...
        for i, eid in enumerate(eids):
            idx_map[eid] = i  # final occurrence becomes final window index
