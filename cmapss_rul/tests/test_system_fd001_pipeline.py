...
def test_fd001_full_pipeline_end_to_end():
    ...
    assert "FD001" in rul_data

    assert isinstance(train_data["FD001"], pd.DataFrame)
    assert not train_data["FD001"].empty

    assert isinstance(test_data["FD001"], pd.DataFrame)
    assert not test_data["FD001"].empty

    assert isinstance(rul_data["FD001"], pd.DataFrame)
    assert len(rul_data["FD001"]) > 0
    ...
    assert "RUL" in train_data["FD001"].columns
    ...
    # Check that core training arrays exist
    assert "X_train" in sequences_data
    assert "y_train" in sequences_data
    assert "X_val" in sequences_data
    assert "y_val" in sequences_data

    # Check that test dictionaries exist (for FD001, etc.)
    assert "X_test_dict" in sequences_data
    assert "y_test_dict" in sequences_data
    assert "engine_ids_test_dict" in sequences_data
    assert "last_idx_map" in sequences_data

    # Optional: sanity check FD001 is present in the test dicts
    assert "FD001" in sequences_data["X_test_dict"]
    assert "FD001" in sequences_data["y_test_dict"]
    assert "FD001" in sequences_data["engine_ids_test_dict"]
